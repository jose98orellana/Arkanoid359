...
startLives:
	...
	ldr	r1, =lives		//r1 = address of lives[0]
	mov	r9, #20			//r9 = initial y coordinate
	mov	r10, #659		//r10 = register for spacing characters/initial x coordinate
loopLives:
...
namesLabel:	
	...
	mov	r9, #800		//r13 = initial y coordinate
	mov	r10, #860		//r11 = register for spacing characters/initial x coordinate
loopNames:
...
startSG:
	...
	mov	r9, #500		//r13 = Initial y coordinate 
	mov	r10, #900		//r11 = register for spacing characters/where x coordinat begins
loopSG:
	ldrb	r11, [r4], #1 		//r12 = byte, post increment by 1, contains current char in array
...
startQG:	
	...
	mov	r9, #700		//r13 = initial y-coordinate
	mov	r10, #900		//r11 = register for spacing characters
loopQG:
	ldrb	r11, [r4], #1 		//r12 = byte, post increment by 1, contains current char in array
...
printCont:
	...
	mov	r9, #500		//r9 = initial y coordinate
	mov	r10, #900		//r10 = register for spacing characters/initial x coordinate
loopCont:
	ldrb	r11, [r1], #1 		//r11 = byte, post increment by 1, contains current char in array
...
printPause:
	...
	mov	r9, #350		//r9 = initial y coordinate
	mov	r10, #940		//r10 = register for spacing characters/initial x coordinate
loopPause:
	ldrb	r11, [r1], #1 		//r11 = byte, post increment by 1, contains current char in array
...
printGameName:
	push	{r9-r11,lr}
	ldr	r0, =endGameName	//r0 = address of EndContine
	ldr	r1, =gameName		//r1 = address of continue[0]
	mov	r9, #300		//r9 = initial y coordinate
	mov	r10, #940		//r10 = register for spacing characters/initial x coordinate
loopGameName:
	ldrb	r11, [r1], #1 		//r11 = byte, post increment by 1, contains current char in array
	...
	cmp	r0, r1			//reached end of array?
	bne 	loopGameName		//loop until reach array end
	popeq 	{r9-r11,lr}
	moveq	pc, lr
...
drawArrow:
	push	{r9-r11,lr}
	ldr	r4, =endArrow		//r0 = address of EndContine
	ldr	r3, =arrow		//r1 = address of continue[0]
	mov	r9, r1		//r9 = initial y coordinate
	mov	r10, r0		//r10 = register for spacing characters/initial x coordinate
loopArrow:
	ldrb	r11, [r3], #1 		//r11 = byte, post increment by 1, contains current char in array
	
	bl	DrawChar		//draw char
	add	r10, #10		//Increment spacing between chars	

	cmp	r4, r3			//reached end of array?
	bne 	loopArrow		//loop until reach array end
	popeq 	{r9-r11,lr}
	moveq	pc, lr
...
arrow:		.byte	'-', '>'						// Array for "->"
endArrow:									//Dummy label for end of array
.align

gameName:	.byte	'A', 'R', 'K', 'A', 'N', 'O', 'I', 'D'			//Array for "Arkanoid"
endGameName:									//Dummy label for end of array
.align

names:		.byte	'M', 'A', 'D', 'E', ' ', 'B', 'Y', ':', ' ', 'J', 'O', 'S', 'E', ' ', 'A', 'N', 'D', ' ', 'Z', 'I', 'L', 'I', 'N'  				//Array for our names
